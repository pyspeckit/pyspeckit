    ...
    def fiteach(self, errspec=None, errmap=None, guesses=(), verbose=True,
        ...
        *guesses*, None
        ...
        if 'multifit' in fitkwargs:
            log.warn("The multifit keyword is no longer required.  All fits allow for multiple components.", log.DeprecatedWarning)
        ...
        def fit_a_pixel(iixy):
            ...
            if sp.error is not None and signal_cut > 0:
                ...
                if max_sn < signal_cut:
                    print 'max_sn < signal_cut, return none'
                    if verbose_level > 1:
                        log.info("Skipped %4i,%4i (s/n=%0.2g)" % (x,y,max_sn))
                    return
                elif np.isnan(max_sn):
                    print 'max_sn is nan, return none'
                    if verbose_level > 1:

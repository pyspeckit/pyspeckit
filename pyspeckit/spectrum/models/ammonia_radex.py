...
from .ammonia_grids import ammonia_grids

## Change file name!!
grid_interpolator = ammonia_grids()
...
def ammonia_radex(xarr, tkin=20, ntot=14, width=1, xoff_v=0.0, logdens = 4.0,
    ...
    radexdict = grid_interpolator(temperature = tkin, column_density = 1e1**ntot,
                                 sigma = width, density = 1e1**logdens)
    numberdict = {'oneone':'11',
                  'twotwo':'22',
                  'threethree':'33',
                  'fourfour':'44'}
    ...
    for linename in line_names:
        ...
        tau_dict[linename] = radexdict['tau_'+numberdict[linename]]
    ...
    for linename in line_names:
        ...
        tex = radexdict['Tex_'+numberdict[linename]]
        # tau array
...
class ammonia_radex_model(model.SpectralModel):

    def __init__(self,npeaks=1,npars=6,
                 parnames=['tkin','ntot','width','xoff_v','logdens','fortho'],
                 **kwargs):
        ...
        self.modelfunc = ammonia_radex
        self.n_modelfunc = self.n_ammonia_radex
        ...
        for par in self.default_parinfo:
            ...
            if 'ntot' in par.parname.lower():
                par.limited = (True,par.limited[1])
                par.limits = (max(par.limits[0],0), par.limits[1])
            if 'logdens' in par.parname.lower():
                par.limited = (True,par.limited[1])
                par.limits = (max(par.limits[0],0), par.limits[1])
    ...
    def __call__(self,*args,**kwargs):
        return self.multinh3fit_radex(*args,**kwargs)

    def n_ammonia_radex(self, pars=None, parnames=None, **kwargs):
        """
    ...
    def components(self, xarr, pars, hyperfine=False, **kwargs):
        ...
        for ii in range(self.npeaks):
            if hyperfine:
                ...
                comps.append( ammonia_radex(xarr,return_components=True,**modelkwargs) )
            else:
                modelkwargs = dict(zip(self.parnames[ii*self.npars:(ii+1)*self.npars],pars[ii*self.npars:(ii+1)*self.npars]))
                comps.append( [ammonia_radex(xarr,return_components=False,**modelkwargs)] )
    ...
    def multinh3fit_radex(self, xax, data, err=None,
                          parinfo=None,
                          quiet=True, shh=True,
                          debug=False,
                          maxiter=200,
        ...
        def mpfitfun(x,y,err):
            if err is None:
                def f(p,fjac=None): return [0,(y-self.n_ammonia_radex(pars=p,
        ...
        self.model = self.n_ammonia_radex(pars=self.mpp, parnames=self.mppnames,
                                    **fitfun_kwargs)(xax)
        ...
        self.tau_list = [ammonia_radex(xax,return_tau=True,**mk) for mk in modelkwargs]
    ...
    def moments(self, Xax, data, negamp=None, veryverbose=False,  **kwargs):
        ...
        """
        return [20, 14, 1.0, 0.0, 3, 0.0]

    def annotations(self):
        from decimal import Decimal # for formatting
        tex_key = {'tkin':'T_K', 'ntot':'N', 'fortho':'F_o',
    ...
    def make_parinfo(self, quiet=True,
                     npeaks=1,
                     params=(20,14,1.0,0.0,3,0.0), parnames=None,
                     fixed=(False,False,False,False,False,False),
                     limitedmin=(True,True,True,True,True,True),
                     limitedmax=(False,False,True,False,True,True),
                     minpars=(TCMB,5,0,0,0,0),
                     maxpars=(0,0,25,0,0,5,1),
                     tied=('',)*6,
                     max_tem_step=1.,
                     **kwargs
                     ):
        ...
        parinfo = [ {'n':ii, 'value':partype_dict['params'][ii],
                     ...
                     'mpmaxstep':max_tem_step*float(partype_dict['parnames'][ii] in ('tkin')), # must force small steps in temperature (True = 1.0)
                     'error': 0}

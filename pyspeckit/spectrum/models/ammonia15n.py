...
from .ammonia15n_constants import (line_names, freq_dict, aval_dict, ortho_dict,
                                voff_lines_dict, tau_wts_dict)

TCMB = 2.7315 # K

def ammonia15n(xarr, trot=20, tex=None, ntot=12, width=1, xoff_v=0.0, fortho=0.0,
    ...
    """
    Generate a model 15-NH3 spectrum based on input temperatures, column, and
    ...
    tex: float or None
        ...
        lines.
    ntot: float
        Total log column density of 15-NH3.  Can be specified as a float in the
        range 5-25
    width: float
    ...
    xoff_v: float
        Line offset in km/s
    fortho: float
        Fraction of 15-NH3 molecules in ortho state.  Default assumes all para
        (fortho=0).
    tau: None or float
    ...
    from .ammonia15n_constants import (ckms, ccms, h, kb,
    ...
    from .ammonia15n_constants import line_name_indices, line_names as original_line_names
    ...
    model_spectrum = _ammonia15n_spectrum(xarr, tex, tau_dict, width, xoff_v,
                                       fortho, line_names,
...
def _ammonia15n_spectrum(xarr, tex, tau_dict, width, xoff_v, fortho, line_names,
                      background_tb=TCMB, fillingfraction=None,
                      return_components=False, return_tau_profile=False):
    """
    Helper function: given a dictionary of 15n-ammonia optical depths,
    ...
    particular 15n-ammonia line being modeled.
    """
    from .ammonia15n_constants import (ckms, h, kb)
...
class ammonia15n_model(model.SpectralModel):
    """
    The basic 15n-Ammonia (15NH3) model with 6 free parameters:
    ...
    def __init__(self,npeaks=1,npars=6,
        ...
        self.n_modelfunc = self.n_ammonia15n

        # for fitting ammonia simultaneously with a flat background
        self.onepeakammonia = fitter.vheightmodel(ammonia15n)
    ...
    def __call__(self,*args,**kwargs):
        return self.multi15nh3fit(*args,**kwargs)

    def n_ammonia15n(self, pars=None, parnames=None, **kwargs):
        """
        ...
        *parnames* [ list ]
        ...
        if len(pars) != len(parnames):
            ...
            if len(pars) % len(parnames) == 0:
                ...
                raise ValueError("Wrong array lengths passed to n_ammonia15n!")
    ...
    def multi15nh3fit(self, xax, data, err=None,
                    parinfo=None,
                    quiet=True, shh=True,
                    debug=False,
                    maxiter=200,
        ...
        """
        Fit multiple 15nh3 profiles (multiple can be 1)
        ...
        data : array
            y axis
        npeaks : int
            How many 15nh3 profiles to fit?  Default 1 (this could supersede onedgaussfit)
        err : array
        ...
        def mpfitfun(x,y,err):
            if err is None:
                def f(p,fjac=None):
                    return [0,(y-self.n_ammonia15n(pars=p,
        ...
        self.model = self.n_ammonia15n(pars=self.mpp, parnames=self.mppnames,
                                    **fitfun_kwargs)(xax)
    ...
    def parse_3par_guesses(self, guesses):
        ...
        For 15-NH3 models, we add in several extra parameters:
            tex = 2.73 * peak
            trot = tex * 2
            fortho = 0.5
            ntot = 12
        ...
        new_guesses = [-1, -1, 12, -1, -1, 0.5] * npeaks
...
class ammonia15n_model_vtau(ammonia15n_model):
    def __init__(self,
                 ...
                 **kwargs):
        super(ammonia15n_model_vtau, self).__init__(parnames=parnames,
                                                 **kwargs)
    ...
    def _validate_parinfo(self,
                          ...
                          required_limits={'trot': [0, None],
                                           ...
                                           'xoff_v': [None,None],
                                           'fortho': [0,1]}):
        supes = super(ammonia15n_model_vtau, self)
    ...
    def make_parinfo(self,
        ...
        parnames=['trot', 'tex', 'tau', 'width', 'xoff_v', 'fortho']
        """
        return super(ammonia15n_model_vtau, self).make_parinfo(params=params,
                                                            fixed=fixed,
...
class ammonia15n_model_background(ammonia15n_model):
    def __init__(self,**kwargs):
        super(ammonia15n_model_background,self).__init__(npars=7,
                                                      parnames=['trot', 'tex',
                                                                'ntot',
                                                                'width',
                                                                'xoff_v',
    ...
    def make_parinfo(self, npeaks=1, err=None,
                     ...
                     quiet=True, shh=True,
                     veryverbose=False, **kwargs):
        return super(ammonia15n_model_background,
    ...
    def multi15nh3fit(self, xax, data, npeaks=1, err=None,
                    params=(20,20,14,1.0,0.0,0.5,TCMB), parnames=None,
                    fixed=(False,False,False,False,False,False,True),
                    limitedmin=(True,True,True,True,False,True,True),
                    limitedmax=(False,False,False,False,False,True,True),
                    ...
                    veryverbose=False, **kwargs):
        return super(ammonia15n_model_background,
                     self).multi15nh3fit(xax, data, npeaks=npeaks, err=err,
...
class ammonia15n_model_restricted_tex(ammonia15n_model):
    ...
    def __init__(self,
                 ...
                 **kwargs):
        super(ammonia15n_model_restricted_tex, self).__init__(npars=7,
                                                           parnames=parnames,
                                                           **kwargs)
    ...
    def n_ammonia15n(self, pars=None, parnames=None, **kwargs):
        ...
        supes = super(ammonia15n_model_restricted_tex, self)
        return supes.n_ammonia15n(pars=pars, parnames=parnames, **kwargs)

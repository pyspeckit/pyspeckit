...
def read_class(filename,  DEBUG=False, apex=False, skip_blank_spectra=False,
    ...
    f.seek(0)
    if version == 'v1':
        file_description = _read_first_record_v1(f)

    else:
        fdesc_code,record_length_words = numpy.fromfile(f, count=2, dtype='int32')
        record_length = record_length_words*4
        #f.seek(0)
        #first_record = numpy.fromfile(f, count=record_length, dtype='int32')
        f.seek(0)
        file_description = _read_first_record_v2(f, record_length_words)
    ...
    while f.tell() < filelen:
        ...
        if IDcode == '\x00\x00\x00\x00':
            ...
            # IDcode will be the first half of BLOC, an int64
            f.seek(startpos)
            if DEBUG: print "IDCODE = ",IDcode," at pos ",startpos
            index = _read_index(f, filetype=filetype, DEBUG=DEBUG)
            if 'XLINE' in index and index['XLINE']:
                #numpy.any([L in index['XLINE'] for L in line]):
                log.debug("Found an index at %i: %s" % (startpos,index))
                indexes.append(index)
                continue
            elif 'CLINE' in index and index['CLINE']:
                log.debug("Found an index at %i: %s" % (startpos,index))
                indexes.append(index)
        ...
        if skip_data:
            ...
            if memmap:
                spectrum = numpy.memmap(f, dtype='float32', mode='r',
                                        shape=nchan)
            else:
                spectrum = numpy.fromfile(f,count=nchan,dtype='float32')
            if DEBUG > 2:

...
params = [(a,b,c) for a in units.unit_type_dict if a not in ('unknown',None)
                  for b in units.unit_type_dict if b not in ('unknown',None)
                  for c in convention]

@pytest.mark.parametrize(('unit_from','unit_to','convention'),params)
def test_convert_units(unit_from,unit_to,convention):
...
@pytest.mark.parametrize(('unit_from','unit_to','convention'),params)
def test_convert_back(unit_from, unit_to,convention):

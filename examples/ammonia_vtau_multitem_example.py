...
# Step 1. Generate a synthetic spectrum.  Already have a real spectrum?  Skip to step 2!
...
xarr22 = pyspeckit.units.SpectroscopicAxis(np.linspace(-40, 40, 100)*u.km/u.s,
                                           velocity_convention='radio',
                                           refX=ammonia_constants.freq_dict['twotwo']).as_unit(u.GHz)
xarr33 = pyspeckit.units.SpectroscopicAxis(np.linspace(-50, 50, 100)*u.km/u.s,
                                           velocity_convention='radio',
...
synthspec = (ammonia.ammonia(xarr, tkin=20, ntot=15, fortho=0.5, xoff_v=0.0, width=1.0) +
...
fitter = ammonia_hf.nh3_vtau_multimodel_generator(['oneone', 'twotwo', 'threethree'])
...
spectrum.plotter.axis.plot(spectrum.xarr, fitter.n_modelfunc(guesses)(spectrum.xarr), 'b')
...
print "Real optical depths of component 1: ",[ammonia.ammonia(xarr, tkin=20, ntot=15, fortho=0.5, xoff_v=0.0, width=1.0, return_tau=True)[x] for x in ['oneone', 'twotwo', 'threethree']]
print "Real optical depths of component 2: ",[ammonia.ammonia(xarr, tkin=50, ntot=14, fortho=0.5, xoff_v=0.0, width=1.0, return_tau=True)[x] for x in ['oneone', 'twotwo', 'threethree']]
...
synthspec = (ammonia.ammonia(xarr, tkin=20, ntot=16, fortho=0.5, xoff_v=0.0, width=1.0) +
             ammonia.ammonia(xarr, tkin=50, ntot=15, fortho=0.5, xoff_v=0.0, width=1.0))
...
print "Real optical depths of component 1: ",[ammonia.ammonia(xarr, tkin=20, ntot=16, fortho=0.5, xoff_v=0.0, width=1.0, return_tau=True)[x] for x in ['oneone', 'twotwo', 'threethree']]
print "Real optical depths of component 2: ",[ammonia.ammonia(xarr, tkin=50, ntot=15, fortho=0.5, xoff_v=0.0, width=1.0, return_tau=True)[x] for x in ['oneone', 'twotwo', 'threethree']]

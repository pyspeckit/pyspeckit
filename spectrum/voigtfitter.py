...
def onedvoigtfit(xax, data, err=None,
    ...
    def mpfitfun(x,y,err):
        if err == None:
            def f(p,fjac=None): return [0,(y-n_voigt(pars=p[1:])(x)+p[0])]
        else:
            def f(p,fjac=None): return [0,(y-n_voigt(pars=p[1:])(x)+p[0])/err]
        return f
    ...
    return mpp,n_voigt(pars=mpp[1:])(xax),mpperr,chi2

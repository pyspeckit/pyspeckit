...
test_parameters = {
        'TKIN': [7.5, 10, 12.5,15],#[7.5,10,12.5,15,17.5,20,25,30,35,40],
        'Tex_frac': [0.5],#np.linspace(0.1,1,4),
        'ntot': np.linspace(15.25,16.75,15), #np.linspace(14,17,5),
        'width': [0.5],
        }
...
#for tk, txf, nt, sig in (parlist):
                ...
                sp = pyspeckit.Spectrum(xarr=xarr, data=fake_spec,
                        err=err,units='K', header=pyfits.Header({'BUNIT':'K'}))
                ...
                if doplot:
                    sp.plot_special = pyspeckit.wrappers.fitnh3.plotter_override
                    fig = pylab.figure()
                    sp.plot_special_kwargs = {'vrange':[-30,30], 'fignum':fig.number, 'figure':fig}
                    sp.plot_special(sp,**sp.plot_special_kwargs)
                    fig.axes[0].set_title(fig.axes[0].get_title()+" $\\tau=%0.1f$" % real_tau['oneone'])
                    fig.axes[1].set_title(fig.axes[1].get_title()+" $\\tau=%0.1f$" % real_tau['twotwo'])
                    pylab.draw()
    ...
    for te in np.unique(insarr[:,1]):
        ...
        pylab.scatter(insarr[OK,2], insarr[OK,0], s=tausarr[OK,0], marker='o', label='Real T$_K$ (T$_X$=%0.1f)' % te)
        pylab.errorbar(outsarr[OK,2], outsarr[OK,0], yerr=errarr[OK,0],
                xerr=errarr[OK,2], linestyle='none', 
                label='Fitted T$_K$ (T$_X$=%0.1f)' % te, color='r')
    ...
    for te in np.unique(insarr[:,1]):
        ...
        if OK.sum() == 0:
            continue
        pylab.scatter(insarr[OK,2], insarr[OK,1], s=tausarr[OK,0], marker='o', label='Real T$_X$')
        pylab.errorbar(outsarr[OK,2], outsarr[OK,1], yerr=errarr[OK,1],
                xerr=errarr[OK,2], linestyle='none', label='Fitted T$_X$', color='r')
    pylab.axis([15,17,te*0.5,te*1.5])
    #pylab.xlabel("Signal to Noise Ratio")
    pylab.xlabel("Column Density")
    pylab.ylabel("Excitation Temperature")
    pylab.title("T=%0.1f" % tk)
    pylab.legend(loc='best')
    ...
    for te in np.unique(insarr[:,1]):
        ...
        if OK.sum() == 0:
            continue
        pylab.scatter(signarr[OK], insarr[OK,2], s=tausarr[OK,0], marker='o', label='Real $N(NH_3)$ (T$_X$=%0.1f)' % te)
        pylab.errorbar(signarr[OK], outsarr[OK,2], yerr=errarr[OK,2],
                linestyle='none', label='Fitted $N(NH_3)$ (T$_X$=%0.1f)' % te,
                color='r')
    pylab.axis([3,20,13.5,17.5])
    pylab.xlabel("Signal to Noise Ratio")
    ...
    for te in np.unique(insarr[:,1]):
        ...
        if OK.sum() == 0:
            continue
        pylab.scatter(tausarr[OK,0], tausarr[OK,2],marker='o', label='Real $\\tau$ (T$_X$=%0.1f)' % te)
        pylab.scatter(tausarr[OK,1], tausarr[OK,3],marker='s', label='Real $\\tau$ (T$_X$=%0.1f)' % te,c='r')
    pylab.plot([tausarr[OK,:2].min(),tausarr[OK,:2].max()],[tausarr[OK,:2].min(),tausarr[OK,:2].max()],'k--')
    pylab.subplot(224).set_xscale('log')
    pylab.subplot(224).set_yscale('log')
    pylab.axis([0,150,0,150])
    pylab.xlabel("Real Optical Depth")
    pylab.ylabel("Fitted Optical Depth")
    pylab.title("T=%0.1f" % tk)
...
for tk in np.unique(insarr[:,0]):
    for te in np.unique(insarr[:,1]):
        ...
        if OK.sum() == 0:
            continue
        pylab.scatter(insarr[OK,2], insarr[OK,0], s=tausarr[OK,0], marker='o', label='Real T$_K$')
        pylab.errorbar(outsarr[OK,2], outsarr[OK,0], yerr=errarr[OK,0],
                xerr=errarr[OK,2], linestyle='none', label='Fitted T$_K$')
pylab.axis([15,17,5,18])
#pylab.xlabel("Signal to Noise Ratio")
pylab.xlabel("Column Density")
pylab.ylabel("Kinetic Temperature")
pylab.title("T=%0.1f" % tk)
